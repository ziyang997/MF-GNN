...
class LightGCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        self.u_g_embeddings_pre_unit = self.process_features(self.u_g_embeddings_pre)
        self.pos_i_g_embeddings_pre = tf.nn.embedding_lookup(self.weights['item_embedding'], self.pos_items)
        self.pos_i_g_embeddings_pre_unit = self.process_features(self.pos_i_g_embeddings_pre)
        self.neg_i_g_embeddings_pre = tf.nn.embedding_lookup(self.weights['item_embedding'], self.neg_items)
        self.neg_i_g_embeddings_pre_unit = self.process_features(self.neg_i_g_embeddings_pre)
        ...
        *********************************************************
        ...
        """
        self.mf_loss, self.emb_loss, self.reg_loss = self.create_bpr_loss(self.u_g_embeddings_pre_unit,
                                                                          self.pos_i_g_embeddings_pre_unit,
                                                                          self.neg_i_g_embeddings_pre_unit)
        self.loss = self.mf_loss + self.emb_loss

        self.opt = tf.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss)

    def process_features(self, features):
        rowsum = np.array(tf.square(features).sum(1))
    ...
    def create_bpr_loss(self, users, pos_items, neg_items):
        ...
        mf_loss = tf.reduce_mean(tf.negative(tf.log_sigmoid(pos_scores)) + tf.negative(tf.log_sigmoid(tf.negative(neg_scores))))
